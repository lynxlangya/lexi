...
> **状态 — v2.2.1 MVP release line。** 阅读器、EPUB 导入、流式翻译、AI 朗读、MenuBar 浮窗、生词本、左右双栏布局都已上线。当前技术预览包已使用 Developer ID 签名并通过 Apple 公证，以 DMG 形式通过 R2 和 Homebrew Cask 分发。
...
[下载 Lexi 2.2.1](https://pub-971ee03b82ad411a9bb26c62a06ca755.r2.dev/lexi/releases/2.2.1/Lexi-2.2.1-installer.dmg)
...
v2.2.1 是当前 MVP 技术预览版本线：它延续 v2.2.0 的加固改进，并修复阅读器跨段选择的背景绘制，确保所有已选段落始终使用同一强调色。Developer ID 签名且通过 Apple 公证的 DMG 构建包继续通过 R2 和 `lynxlangya/tap` 提供。
