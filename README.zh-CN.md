...
> **状态 — v2.1.1 MVP release line。** 阅读器、EPUB 导入、流式翻译、AI 朗读、MenuBar 浮窗、生词本、左右双栏布局都已上线。当前技术预览包已使用 Developer ID 签名并通过 Apple 公证，以 DMG 形式通过 R2 和 Homebrew Cask 分发。
...
v2.1.1 是当前 MVP 技术预览版本线：计划中的 PR 1–10 已合入，后续修复、OpenAI TTS 支持和朗读 UI 迭代也已落地，并已通过 R2 和 `lynxlangya/tap` 提供 Developer ID 签名且通过 Apple 公证的 DMG 构建包。
