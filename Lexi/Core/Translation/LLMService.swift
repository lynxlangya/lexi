...
actor LLMService {
    ...
    struct Configuration {
        ...
        var promptStrategy: any TranslationPromptStrategy
    ...
    private func makeChatCompletionsRequest(
        ...
        let systemPrompt = makeSystemPrompt(configuration: configuration, sourceText: sourceText)
        let body = ChatCompletionRequest(
            model: configuration.model,
            stream: stream,
            messages: [
                .init(role: "system", content: systemPrompt),
                .init(role: "user", content: configuration.promptStrategy.userPrompt(text: sourceText))
            ]
        )
    ...
    private func makeSystemPrompt(configuration: Configuration, sourceText: String) -> String {
        if let sourceAware = configuration.promptStrategy as? any SourceAwareTranslationPromptStrategy {
            return sourceAware.systemPrompt(
                source: configuration.sourceLanguage,
                target: configuration.targetLanguage,
                text: sourceText
            )
        }
        return configuration.promptStrategy.systemPrompt(
            source: configuration.sourceLanguage,
            target: configuration.targetLanguage
        )
    }
