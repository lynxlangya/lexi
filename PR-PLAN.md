...
- **历史修订**：早期计划曾要求 DEBUG-only `.env.local` / `DevSecrets.swift` 注入真实 key。该路径已废弃；运行时必须统一走 Settings → 引擎，API key 存 Keychain，model / last test status 存 SQLite `EngineConfig`。
